...
def run():
    ...
    for idx, hist_row in df_historical.iterrows():
        ...
        if imputed_fields:
            ...
            # Calculate derived metrics from imputed values for grading
            ...
            if pd.notna(total_ghg) and pd.notna(gross_floor_area) and gross_floor_area > 0:
                ...
                ghg_intensity = (total_ghg * 1000) / gross_floor_area
                df_historical.at[idx, 'GHGIntensity'] = round(ghg_intensity, 2)
                # Add GHGIntensity to imputed fields list if TotalGHGEmissions was imputed
                if 'TotalGHGEmissions' in imputed_fields:
                    imputed_fields.append('GHGIntensity')
                    df_historical.at[idx, 'ImputedFields'] = ','.join(imputed_fields)
